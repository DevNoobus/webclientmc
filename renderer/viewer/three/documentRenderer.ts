...
export class DocumentRenderer {
  ...
  private lastPixelRatioCap: number | undefined = undefined
  ...
  updatePixelRatio () {
    ...
    // When a renderPixelRatio is configured, use it directly so values like 0.5
    // actually render below native resolution (not just cap at a maximum).
    if (this.config.renderPixelRatio !== undefined) {
      pixelRatio = this.config.renderPixelRatio
    }
  ...
  private startRenderLoop () {
    const animate = () => {
      ...
      // Re-apply pixel ratio if the cap setting changed (e.g. user changed it in options)
      if (this.lastPixelRatioCap !== this.config.renderPixelRatio) {
        this.lastPixelRatioCap = this.config.renderPixelRatio
        this.updatePixelRatio()
      }
