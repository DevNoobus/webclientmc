...
export class Entities {
  ...
  setNametags (visible: boolean, entity: THREE.Object3D | null = null) {
    this.currentlyShowingNametags = visible
    const targets = entity ? [entity] : Object.values(this.entities)
    for (const ent of targets) {
      ent.traverse(c => {
        if (c.name === 'nametag') c.visible = visible
      })
    }
  }

  raycastEntity (reach = 6): { entityId: number; x: number; y: number; z: number } | null {
    const raycaster = new THREE.Raycaster()
    raycaster.setFromCamera(new THREE.Vector2(0, 0), this.worldRenderer.camera)
    raycaster.far = reach
    const entityGroups = Object.values(this.entities)
    const intersects = raycaster.intersectObjects(entityGroups, true)
    for (const intersect of intersects) {
      if (intersect.object.name === 'nametag') continue
      let obj: THREE.Object3D | null = intersect.object
      while (obj) {
        if (obj.name === 'entity' && (obj as SceneEntity).originalEntity) {
          const ent = (obj as SceneEntity).originalEntity
          return { entityId: ent.id, x: intersect.point.x, y: intersect.point.y, z: intersect.point.z }
        }
        obj = obj.parent
      }
    }
  ...
  render () {
    ...
    const showNametagsConfig = this.worldRenderer.worldRendererConfig.showNametags
    if (showNametagsConfig !== this.currentlyShowingNametags) {
      this.setNametags(showNametagsConfig)
    }
  ...
  update (entity: SceneEntity['originalEntity'], overrides) {
    ...
    if (e === undefined) {
      ...
      if (entity.name === 'item' || entity.name === 'tnt' || entity.name === 'falling_block' || entity.name === 'snowball'
        ...
        } else if (entity.metadata?.[3] === true && entity.metadata?.[2]) {
          // NPC player with no tab-list username — show their custom_name metadata label
          const customNameStr = entity.metadata[2]
          const labelMeta = { ...entity, username: typeof customNameStr === 'string' ? mojangson.simplify(mojangson.parse(customNameStr)) : nbt.simplify(customNameStr) }
          const nametag = addNametag(labelMeta, { fontFamily: 'mojangles' }, wrapper, this.worldRenderer.version)
          if (nametag) {
            nametag.position.y = playerObject.position.y + playerObject.scale.y * 16 + 3
            nametag.scale.multiplyScalar(12)
          }
      ...
      if (!mesh) {
        // Allow invisible holograms (e.g. Hypixel armor-stand labels) that have NO renderable
        // geometry to still be tracked so their custom_name nametag can be shown. We check both
        // the key-name proxy AND the standard raw metadata indices (2 = custom_name,
        // 3 = custom_name_visible) as a fallback because metadataKeys may not be populated yet.
        const metaEarlyCheck = getGeneralEntitiesMetadata(entity)
        const hasCustomName =
          (metaEarlyCheck.custom_name_visible && metaEarlyCheck.custom_name) ||
          (entity.metadata?.[3] === true && entity.metadata?.[2])
        if (!hasCustomName) return
        const placeholder = new THREE.Object3D()
        mesh = placeholder as unknown as THREE.Mesh
      }
      mesh.name = 'mesh'
    ...
    // Use the key-name proxy first; fall back to standard Minecraft metadata indices
    // (index 2 = custom_name, index 3 = custom_name_visible) for servers/versions where
    // metadataKeys may not include the base-entity keys.
    const customNameRaw = meta.custom_name ?? entity.metadata?.[2]
    const customNameVisible = meta.custom_name_visible ?? entity.metadata?.[3]
    const displayTextRaw = textDisplayMeta?.text || (customNameVisible && customNameRaw)
    // Show metadata-based custom name for all non-player entities, AND for player entities
    // that lack a username (i.e. NPC players not in the tab list, e.g. Hypixel lobby NPCs).
    if (displayTextRaw && (entity.name !== 'player' || !entity.username)) {
