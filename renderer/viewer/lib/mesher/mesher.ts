...
// Optimisation #4: per-section cooldown – avoid re-meshing the same section more than ~10×/sec
// during burst block updates (e.g. pistons, explosions).  The section stays in dirtySections and
// is processed on the next interval tick once the cooldown expires.
const lastMeshedTime = new Map<string, number>()
const SECTION_REMESH_COOLDOWN_MS = 100
...
const handleMessage = data => {
  ...
  switch (data.type) {
    ...
    case 'unloadChunk': {
      ...
      // Clean up cooldown timestamps for unloaded sections so memory doesn't grow unboundedly
      for (const key of lastMeshedTime.keys()) {
        const [kx, , kz] = key.split(',').map(Number)
        if (kx === data.x && kz === data.z) lastMeshedTime.delete(key)
      }
...
setInterval(() => {
  ...
  for (const key of dirtySections.keys()) {
    // Optimisation #4: section cooldown – skip sections meshed very recently to absorb burst updates
    const lastMesh = lastMeshedTime.get(key)
    if (lastMesh !== undefined && now - lastMesh < SECTION_REMESH_COOLDOWN_MS) continue
