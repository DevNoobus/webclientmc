...
function renderElement (world: World, cursor: Vec3, element: BlockElement, doAO: boolean, attr: MesherGeometryOutput, globalMatrix: any, globalShift: any, block: Block, biome: string) {
  ...
  for (const face in element.faces) {
    ...
    for (const pos of corners) {
      ...
      let light = 1
      // Optimisation #8: disable smooth lighting for mid/far sections (LOD)
      const smoothLighting = world.config.smoothLighting && !sectionLodDisableSmoothLighting
      if (doAO) {
        const dx = pos[0] * 2 - 1
        const dy = pos[1] * 2 - 1
        const dz = pos[2] * 2 - 1
...
// Optimisation #8 (LOD): module-level flags set once per section based on distance to viewer.
// Using module-level vars avoids threading new params through renderElement.
let sectionLodDisableAO = false
let sectionLodDisableSmoothLighting = false
// Thresholds in blocks (chunk = 16 blocks)
const LOD_DISABLE_AO_DIST_SQ = (5 * 16) ** 2          // >5 chunks: skip AO entirely
const LOD_DISABLE_SMOOTH_DIST_SQ = (3 * 16) ** 2      // >3 chunks: skip smooth lighting
...
export function getSectionGeometry (sx: number, sy: number, sz: number, world: World) {
  ...
  // Optimisation #8: compute LOD flags for this section based on viewer distance
  const viewerX = (world.config as any).viewerX ?? 0
  const viewerZ = (world.config as any).viewerZ ?? 0
  const sectionCenterX = sx + 8
  const sectionCenterZ = sz + 8
  const distSq = (sectionCenterX - viewerX) ** 2 + (sectionCenterZ - viewerZ) ** 2
  sectionLodDisableAO = distSq > LOD_DISABLE_AO_DIST_SQ
  sectionLodDisableSmoothLighting = distSq > LOD_DISABLE_SMOOTH_DIST_SQ
    ...
    for (cursor.z = sz; cursor.z < sz + 16; cursor.z++) {
      for (cursor.x = sx; cursor.x < sx + 16; cursor.x++) {
        ...
        if (block.name !== 'water' && block.name !== 'lava' && !INVISIBLE_BLOCKS.has(block.name)) {
          // Optimisation #5: skip fully-buried solid cube blocks (all 6 face-neighbours are solid cubes).
          // This is correct because solid cubes completely occlude any block they face.
          if (!isWaterlogged && isCube(block)) {
            const n1 = world.getBlock(cursor.offset(1, 0, 0))
            const n2 = world.getBlock(cursor.offset(-1, 0, 0))
            const n3 = world.getBlock(cursor.offset(0, 1, 0))
            const n4 = world.getBlock(cursor.offset(0, -1, 0))
            const n5 = world.getBlock(cursor.offset(0, 0, 1))
            const n6 = world.getBlock(cursor.offset(0, 0, -1))
            if (n1 && n2 && n3 && n4 && n5 && n6 &&
                isCube(n1) && isCube(n2) && isCube(n3) && isCube(n4) && isCube(n5) && isCube(n6)) {
              attr.blocksCount++ // still counted, just not rendered
              continue
            }
          }
          ...
          for (const modelVars of models ?? []) {
            ...
            for (const element of model.elements ?? []) {
              // Optimisation #8: disable AO for sections far from the viewer
              const ao = (model.ao ?? block.boundingBox !== 'empty') && !sectionLodDisableAO
