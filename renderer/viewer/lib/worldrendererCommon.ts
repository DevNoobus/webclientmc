...
export const defaultWorldRendererConfig = {
  ...
  // Scale worker count with available CPU cores (leave 1 core for the main thread, cap at 8)
  mesherWorkers: typeof navigator !== 'undefined' && navigator.hardwareConcurrency
...
export abstract class WorldRendererCommon<WorkerSend = any, WorkerReceive = any> {
  ...
  ONMESSAGE_TIME_LIMIT = 12 // ms – stay well under one 60fps frame (16ms) to keep rendering smooth
  ...
  getMesherConfig (): MesherConfig {
    ...
    return {
      ...
      // LOD: pass viewer position so mesher workers can reduce quality for distant sections
      viewerX: this.viewerChunkPosition?.x ?? 0,
      viewerZ: this.viewerChunkPosition?.z ?? 0,
    }
  }
  ...
  dispatchMessages () {
    ...
    // Back-pressure (#10): when the main-thread geometry queue is large the workers are already ahead;
    // delay the next batch of dirty messages so we don't pile up more unprocessed geometry.
    const backPressureDelay = this.messageQueue.length > 150 ? 80 : 0
    setTimeout(() => {
