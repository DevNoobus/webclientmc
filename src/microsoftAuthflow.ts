export const getProxyDetails = async (proxyBaseUrl: string) => {
  ...
  console.log('[MSAuth] fetching proxy capabilities from:', url)
  let result: Response
  try {
  ...
  console.log('[MSAuth] proxy capabilities status:', result.status)
  return result
}
...
export default async ({ tokenCaches, proxyBaseUrl, setProgressText = (text) => { }, setCacheResult, connectingServer }) => {
  ...
  try {
    ...
    console.log('[MSAuth] capabilities JSON:', JSON.stringify(json))
    ...
    console.log('[MSAuth] authEndpoint:', authEndpoint?.toString(), 'sessionEndpoint:', sessionEndpoint?.toString())
    if (!authEndpoint) throw new Error('No auth endpoint')
  } catch (err) {
    console.error('[MSAuth] failed to parse capabilities:', err)
    throw new Error(`Selected proxy server ${proxyBaseUrl} does not support Microsoft authentication`)
  }
  const authFlow = {
    async getMinecraftJavaToken () {
      console.log('[MSAuth] getMinecraftJavaToken called, posting to:', authEndpoint?.toString())
      ...
      await fetch(authEndpoint, {
        ...
        .then(async response => {
          console.log('[MSAuth] fetch response arrived, status:', response.status, 'ok:', response.ok)
          if (!response.ok) {
          ...
          const processText = ({ done, value = undefined as Uint8Array | undefined }) => {
            ...
            const processChunk = (chunkStr) => {
              ...
              console.log('[MSAuth] chunk received:', chunkStr.slice(0, 200))
              let json: any
              try {
              ...
              if (!json) return
              if (json.user_code) {
                console.log('[MSAuth] device code chunk, calling onMsaCodeCallback, defined:', typeof onMsaCodeCallback)
                onMsaCodeCallback(json)
                // this.codeCallback(json)
              }
              if (json.error) {
                const errorStr = String(json.error)
                if (errorStr.includes('403') && (errorStr.includes('{}') || errorStr.includes('Forbidden'))) {
                  throw new Error('This Microsoft account does not own Minecraft Java Edition. Please use an account with a valid Java Edition license.')
                }
                throw new Error(`Auth server error: ${json.error}`)
              }
