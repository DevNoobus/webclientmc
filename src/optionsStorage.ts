...
// Auto-detect low-end devices (e.g. Chromebooks with 2 GB RAM) and apply conservative
// defaults on first launch (before the user has changed anything).
// Only fires once — if changedSettings is already populated the user has made choices.
if (Object.keys(appStorage.changedSettings).length === 0) {
  const mem = (navigator as any).deviceMemory as number | undefined
  const cores = navigator.hardwareConcurrency ?? 4
  const dpr = window.devicePixelRatio ?? 1
  const isLowEnd = (mem !== undefined && mem <= 2) || cores <= 2 || (mem !== undefined && mem <= 4 && dpr >= 2)
  if (isLowEnd) {
    Object.assign(appStorage.changedSettings, {
      renderPixelRatio: '1',
      smoothLighting: false,
      dayCycleAndLighting: false,
      renderEntities: false,
      lowMemoryMode: true,
      numWorkers: 2,
      multiplayerRenderDistance: 2,
      renderDistance: 2,
      loadPlayerSkins: false,
      starfieldRendering: false,
      frameLimit: 30,
    } satisfies Partial<typeof defaultOptions>)
  }
}
