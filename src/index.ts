  ...
  try {
    ...
    bot = mineflayer.createBot({
      ...
      auth: connectOptions.authenticatedAccount ? async (client, options) => {
        // createClient.js fires auth(client,options) without await then calls onReady().
        // For stream (WebSocket) connections index.ts emits bot._client('connect') right
        // after createBot() returns – before this async function has set client.username.
        // wait_connect blocks login_start; we intercept options.connect so that when
        // protocolMicrosoftAuth.authenticate() finishes it unblocks login_start instead
        // of re-establishing the already-open stream socket.
        if (options.stream) {
          client.wait_connect = true
          options.connect = () => { client.emit('connect_allowed') }
        }

        authData!.setOnMsaCodeCallback(options.onMsaCode)
        authData?.setConnectingVersion(client.version)
        // Inject our proxy-backed authflow – protocolMicrosoftAuth.authenticate will call
        // client.authflow.getMinecraftJavaToken() which routes through our /auth/ms proxy.
        // @ts-expect-error
        ...
        try {
          ...
          // Refresh the stored cache tokens so the next connection reuses them
          if (typeof connectOptions.authenticatedAccount === 'object' && client.username) {
            updateAuthenticatedAccountData(accounts => {
              const existing = accounts.find(a => a.username === client.username)
              if (existing) existing.cachedTokens = { data: newTokensCacheResult, expiresOn: Date.now() + 86_400_000 }
              return accounts
            })
          }
          updateDataAfterJoin = () => {
