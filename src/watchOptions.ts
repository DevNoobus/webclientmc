...
export const watchOptionsAfterViewerInit = () => {
  ...
  // Individual stored options are NOT changed — disabling restores them.
  const applyPotatoMode = (enabled: boolean, o: typeof options) => {
    if (enabled) {
      ...
      appViewer.inWorldRenderingConfig.showNametags = false
    } else {
      appViewer.config.renderPixelRatio = o.renderPixelRatio === 'auto' ? undefined : parseFloat(o.renderPixelRatio)
  ...
  subscribeKey(options, 'potatoMode', () => {
