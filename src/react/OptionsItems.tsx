...
export const OptionButton = ({ item, onClick, valueText, cacheKey }: {
  ...
  return <Button
    ...
    label={`${translate(item.text)}: ${translate(valueText ?? valuesTitlesMap[optionValue] ?? String(optionValue))}`}
    onClick={async (event) => {
      if (disabledReason) {
      ...
      if (item.id) {
        ...
        if (possibleValues && possibleValues.length >= 4) {
          ...
          // For options with known possible values, cycle through them (also handles single-value reset)
