...
import { addMicrosoftAccount } from '../addMicrosoftAccount'
import RealmsModal from './RealmsModal'
...
const Inner = ({ hidden, customServersList }: { hidden?: boolean, customServersList?: string[] }) => {
  ...
  const serversListJsx = <ServersList
    ...
    onAddAccountClick={async () => {
      ...
      if (!proxy) {
        showNotification('Set a proxy server first (use localhost:8080 for local auth)', '', true)
        return
      }
      try {
        const username = await addMicrosoftAccount(proxy)
        showNotification(`Signed in as ${username}`)
      } catch (err: any) {
        if (err?.message !== 'Cancelled by user') {
          showNotification('Authentication failed: ' + (err?.message ?? String(err)), '', true)
        }
      }
    }}
    onRealmsClick={() => setRealmsOpen(true)}
  ...
  return <>
    ...
    {realmsOpen && (
      <RealmsModal
        onClose={() => setRealmsOpen(false)}
        authenticatedAccounts={authenticatedAccounts}
        onConnectRealm={(host, port, platform) => {
          if (platform === 'bedrock') {
            showNotification(`Bedrock Realm address: ${host}:${port} (Bedrock not playable here)`, '', true)
            return
          }
          const ip = `${host}:${port}`
          ...
          const options: ConnectOptions = {
            ...
            ignoreQs: true,
            authenticatedAccount: account ?? true,
          }
          dispatchEvent(new CustomEvent('connect', { detail: options }))
        }}
      />
