...
const domListeners = (bot: Bot) => {
  ...
  document.addEventListener('mousedown', async (e) => {
    ...
    if (e.button === 0) {
      const entityHit = await getThreeJsRendererMethods()?.raycastEntity()
      if (entityHit) {
        bot._client.write('use_entity', {
          target: entityHit.entityId,
          mouse: 1, // attack
      ...
      if (!cursorState?.cursorBlock) {
        // Nothing in range — punch the air: send arm swing so the server sees it
